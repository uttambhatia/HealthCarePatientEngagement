Param(
    [string]$Namespace = "healthcare-prod-dr",
    [string]$KubeContext = "",
    [string]$EnvFile = "",
    [string]$SecretFile = "",
    [switch]$SkipGuardrails,
    [switch]$AcknowledgeDr,
    [switch]$Preflight
)

$ErrorActionPreference = "Stop"

if ((-not $Preflight) -and (-not $AcknowledgeDr)) {
    throw "Secondary DR rollout requires -AcknowledgeDr."
}
...
$defaultSecretFile = Join-Path $scriptDir "platform-secrets.secondary.template.yaml"
...
if ($EnvFile -ne "") {
    ...
    $secretFile = Join-Path $scriptDir "platform-secrets.secondary.generated.yaml"

    Write-Host "Rendering secondary secret manifest from $EnvFile"
    try {
        & $renderScript -EnvFile $EnvFile -OutputFile $secretFile -Namespace $Namespace
    }
    catch {
        throw "Failed to render secondary secret manifest from env file."
    }
}
...
Write-Host "Validating secondary secret template completeness"
...
Write-Host "Applying secondary platform secrets"
...
Write-Host "Secondary DR rollout completed."
