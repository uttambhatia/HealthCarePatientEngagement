...
public class ServiceBusAdapter {
    ...
    public ServiceBusAdapter(
            ...
            @Value("${platform.integration.service-bus.base-url:}") String baseUrl,
            @Value("${platform.integration.service-bus.path:/servicebus/messages}") String path,
    ...
    public void queueOnBus(ServiceBusMessageRecord aggregate, String correlationId) {
        if (restClient == null) {
            LOGGER.warn("Skipping service bus integration for aggregateId={} because base URL is not configured", aggregate.id());
            return;
        }
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                restClient.post()
                        ...
                        .retrieve()
                        .toBodilessEntity();
                LOGGER.info("Service bus integration succeeded aggregateId={} correlationId={} attempt={}", aggregate.id(), correlationId, attempt);
                return;
            } catch (RuntimeException ex) {
                lastError = ex;
                LOGGER.warn("Service bus integration attempt failed aggregateId={} correlationId={} attempt={} maxAttempts={} error={}",
                        aggregate.id(), correlationId, attempt, maxAttempts, ex.getMessage());
            }
        }

        throw new IllegalStateException("Service bus integration failed after retries for aggregateId=" + aggregate.id(), lastError);
    }
}
