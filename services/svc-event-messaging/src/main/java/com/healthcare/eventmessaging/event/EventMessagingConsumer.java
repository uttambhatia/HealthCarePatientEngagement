package com.healthcare.eventmessaging.event;

import com.healthcare.platform.common.event.MessageEnvelope;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Component;

@Component
public class EventMessagingConsumer {
    private static final Logger LOGGER = LoggerFactory.getLogger(EventMessagingConsumer.class);

    public void handle(MessageEnvelope<ServiceBusMessageQueuedEvent> envelope) {
        LOGGER.info("Consumed eventName={} correlationId={}", envelope.eventName(), envelope.correlationId());
    }
}
